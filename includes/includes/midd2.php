...
$u=$_SESSION['username'];
$q="SELECT * FROM bookentries where subject='Computer Science' and Book_Name NOT IN (Select Book_Name from subscriber where Username = '$u')";
$result=$db->query($q);

if ($result->num_rows ==0) 
  {
    echo "<center>There are no more books available for this subject.<br><br>";
  }
...
<?php
$db=mysqli_connect('localhost','root','password','registration');
$q="SELECT * FROM bookentries where subject='Electronics' and Book_Name NOT IN (Select Book_Name from subscriber where Username = '$u')";
...
<?php
$db=mysqli_connect('localhost','root','password','registration');
$q="SELECT * FROM bookentries where subject='Mechanical' and Book_Name NOT IN (Select Book_Name from subscriber where Username = '$u')";
...
$result=$db->query($q);
 if ($result->num_rows ==0) 
  {
    echo "<center>There are no more books available for this subject.</center><br><br>";
   }
//$query=mysql_query('select * from new_post');
while($row = $result->fetch_assoc())
{
